alias py='python'
alias netstat='ss'
alias yayy='sudo timeshift --create; yay'
alias gophp='cd ~/codes/php'
alias xamp='sudo xampp start' 
alias ahirusic='mpv --shuffle /ahirut/ahiru/Music/'
alias yotob='yt-dlp $@'
alias bat-timeline='cat /var/lib/upower/history-charge-ASUS_Battery-42.dat | while read f; do   d=$(date +"%b %e %H:%M:%S" -s @`echo $f | cut -d\  -f1`);   echo "$d  $f" ; done'


#combine all pdf ghostscript:
compdf(){
gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile="$@";
#if [ $? -eq 0 ]; then
#	read -p "Remove $@?: " remove 
#	if [$remove == "" ] || [$remove == "y"] || [$remove  == "Y"]; then
#		eval "rm "$@"";
#	fi
#fi
}
...
#youtube-dl related:
dlmp3(){
yt-dlp --extract-audio --audio-format mp3 -o "/ahirut/ahiru/Music/%(title)s.%(ext)s" "$@"; 
}
...
mp3search(){
	yt-dlp --extract-audio --audio-format mp3 -o "/ahirut/ahiru/Music/%(title)s.%(ext)s" ytsearch:"$@";
}
...
#add class related links mon, tue... for module links
